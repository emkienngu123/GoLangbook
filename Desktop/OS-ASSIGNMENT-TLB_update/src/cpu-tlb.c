...
int tlbfree_data(struct pcb_t *proc, uint32_t reg_index)
{
  ...
  struct vm_rg_struct *freerg_node = proc->mm->mmap->vm_freerg_list;

  if (freerg_node == NULL)
    return -1;

  int addr = freerg_node->rg_start;

  int pgn = PAGING_PGN(addr);
  uint32_t pte = proc->mm->pgd[pgn];
  int frmnum = PAGING_FPN(pte);

  if (tlb_cache_read(proc->tlb, proc->pid, pgn, &frmnum) != -1) 
    tlb_flush_entry(proc->tlb, proc->pid, pgn);
  //oke
  TLBMEMPHY_dump(proc->tlb);
  print_pgtbl(proc, 0, -1);
