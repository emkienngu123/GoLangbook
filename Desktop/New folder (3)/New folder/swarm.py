...
class swarm():
    ...
    def download_using_stratergies(self):
        ...
        if self.torrent.torrent_metadata.files is None:
            for i in range(self.torrent.pieces_count):
                dir_path = 'downloaded'

                with open('output/piece'+str(i),'rb') as f:
                    data = f.read()
                with open(dir_path+"/"+ self.torrent.torrent_metadata.file_name , 'ab') as file:
                    file.write(data)
            ...
            for i in range(self.torrent.pieces_count):
                ...
                with open('output/piece'+str(i),'rb') as f:
                    data = f.read()
                with open(self.torrent.torrent_metadata.file_name , 'ab') as file:
                    file.write(data)

            with open(self.torrent.torrent_metadata.file_name , 'rb') as f:
                byte_stream = f.read()
            file_length = [x[0] for x in self.torrent.torrent_metadata.files]
            file_names = [x[1] for x in self.torrent.torrent_metadata.files]
            file_mark = np.cumsum([0] + file_length)

            for i in range(len(file_names)):
                with open(dir_path + "/" + file_names[i] , 'wb') as f:
                    f.write(byte_stream[file_mark[i]:file_mark[i+1]])
